...
try:
    ...
    logging.warning(e)
    logging.warning(
        "Could not import a non-standard Python module needed by the WorldMap class, or the version of the non-standard module is too old. Check that all the PyQSO dependencies are satisfied."
    )
...
if have_necessary_modules:
    ...
    class NavigationToolbar(NavigationToolbar2GTK3):
        ...
        toolitems = [
            t
            for t in NavigationToolbar2GTK3.toolitems
            if t[0] in ("Home", "Zoom", "Save")
        ]
...
class Maidenhead:
    ...
    def ll2gs(self, latitude, longitude, subsquare=False):
        ...
        if subsquare:
            adjusted_latitude_remainder = (
                adjusted_latitude - int(adjusted_latitude)
            ) * 60
            adjusted_longitude_remainder = (
                (adjusted_longitude) - int(adjusted_longitude / 2) * 2
            ) * 60
    ...
    def gs2ll(self, grid_square):
        ...
        if m:
            ...
            if m:
                gs = m.group(0)
                latitude = self.latitude4(gs) + (1.0 / 60.0) * 2.5 * (
                    ord(gs[5]) - ord("a") + 0.5
                )
                longitude = self.longitude4(gs) + (1.0 / 60.0) * 5 * (
                    ord(gs[4]) - ord("a") + 0.5
                )
            else:
...
class WorldMap:
    ...
    def __init__(self, application):
        ...
        if have_necessary_modules:
            ...
            self.refresh_event = GObject.timeout_add(
                1800000, self.draw
            )  # Re-draw the world map automatically after 30 minutes (if the world map tool is visible).
        ...
        if have_config and config.has_option(section, option):
            if config.getboolean(section, option):
                try:
                    ...
                    logging.warning(
                        "Unable to get the QTH name, latitude and/or longitude. The QTH will not be pinpointed on the world map. Check preferences?"
                    )
        ...
        if have_geocoder:
            ...
            if country:
                try:
                    ...
                    logging.exception(
                        "Unable to lookup QTH coordinates. Check connection to the internets? Lookup limit reached?"
                    )
    ...
    def get_worked_grid_squares(self, logbook):
        ...
        worked_grid_squares = numpy.zeros(
            (len(self.maidenhead.upper), len(self.maidenhead.upper)), dtype=bool
        )
                ...
                for r in records:
                    if r["GRIDSQUARE"]:
                        grid_square = r["GRIDSQUARE"][
                            0:2
                        ].upper()  # Only consider the field value (e.g. IO).
                        worked_grid_squares[
                            self.maidenhead.upper.index(grid_square[1]),
                            self.maidenhead.upper.index(grid_square[0]),
                        ] = True

            except sqlite.Error as e:
                logging.error(
                    "Could not update the array of worked grid squares for log '%s' because of a database error."
                    % log.name
                )
                logging.exception(e)
        ...
        if have_necessary_modules:
            ...
            if tools.get_current_page() != 1 or not toolbox.get_visible():
                ...
                days_since_reference = (
                    dt - reference_solstice
                ).total_seconds() / seconds_per_day
                latitude = axial_tilt * numpy.cos(
                    2 * numpy.pi * days_since_reference / days_per_year
                )
                ...
                y = numpy.empty(360)
                x[:180] = -90
                y[:180] = numpy.arange(-90, 90.0)
                x[180:] = 90
                y[180:] = numpy.arange(90, -90.0, -1)
                ...
                if self.points:
                    ...
                    for p in self.points:
                        ...
                        projected_x, projected_y = ax.projection.transform_point(
                            p.longitude, p.latitude, src_crs=cartopy.crs.PlateCarree()
                        )
                        ax.annotate(
                            p.name,
                ...
                if self.show_grid_squares:
                    if self.shade_worked_grid_squares:
                        worked_grid_squares = self.get_worked_grid_squares(
                            self.application.logbook
                        )
                        masked = numpy.ma.masked_array(
                            worked_grid_squares, worked_grid_squares == 0
                        )
                    else:
                        z = numpy.zeros(
                            (len(self.maidenhead.upper), len(self.maidenhead.upper)),
                            dtype=bool,
                        )
