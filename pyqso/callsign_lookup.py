...
class CallsignLookupQRZ:
    ...
    def connect(self, username, password):
        ...
        try:
            ...
            request = "/xml/current/?username=%s;password=%s;agent=pyqso" % (
                username,
                quote(password),
            )  # Percent-escape the password in case there are reserved characters present.
    ...
    def lookup(self, full_callsign, ignore_prefix_suffix=True):
        ...
        logging.debug(
            "Looking up callsign. The full callsign (with a prefix and/or suffix) is %s."
            % full_callsign
        )
        ...
        if self.session_key:
            ...
            if callsign_node:
                callsign_node = callsign_node[
                    0
                ]  # There should only be a maximum of one Callsign element.
                ...
                if callsign_fname_node:
                    fields_and_data["NAME"] = callsign_fname_node[
                        0
                ...
                if callsign_name_node:  # Add the surname, if present.
                    fields_and_data["NAME"] = (
                        fields_and_data["NAME"]
                        + " "
                        + callsign_name_node[0].firstChild.nodeValue
                    )
...
class CallsignLookupHamQTH:
    ...
    def connect(self, username, password):
        ...
        if session_id_node:
            ...
            logging.debug(
                "Successfully connected to the hamqth.com server. Session ID is: %s."
                % self.session_id
            )
            connected = True
        else:
    ...
    def lookup(self, full_callsign, ignore_prefix_suffix=True):
        ...
        if self.session_id:
            request = "/xml.php?id=%s&callsign=%s&prg=pyqso" % (
                self.session_id,
                callsign,
            )
            self.connection.request("GET", request)
            ...
            if search_node:
                ...
                if search_addr1_node:
                    fields_and_data["ADDRESS"] = search_addr1_node[
                        0
                    ].firstChild.nodeValue
                if search_addr2_node:  # Add the second line of the address, if present.
                    fields_and_data["ADDRESS"] = (
                        fields_and_data["ADDRESS"] + ", " if search_addr1_node else ""
                    ) + search_addr2_node[0].firstChild.nodeValue
                ...
                if search_country_node:
                    fields_and_data["COUNTRY"] = search_country_node[
                        0
                    ].firstChild.nodeValue
                ...
                if search_ituzone_node:
                    fields_and_data["ITUZ"] = search_ituzone_node[
                        0
                    ].firstChild.nodeValue
...
def strip(full_callsign):
    ...
    components = full_callsign.split(
        "/"
    )  # We assume that prefixes or suffixes come before/after a forward slash character "/".
    ...
    try:
        if len(components) == 3:
            ...
            if components[1].upper() in suffixes or components[1].lower() in suffixes:
                ...
                logging.debug(
                    "Suffix %s found. Callsign to lookup is %s."
                    % (components[1], callsign)
                )
            else:
                # We have a prefix, so take the part after the first "/".
                callsign = components[1]
                logging.debug(
                    "Prefix %s found. Callsign to lookup is %s."
                    % (components[0], callsign)
                )
            ...
            callsign = full_callsign
            logging.debug(
                "No prefix or suffix found. Callsign to lookup is %s." % callsign
            )
