...
class Logbook:
    ...
    def delete_log(self, widget, page=None):
        ...
        if page is None:
            ...
            if page_index == 0:  # If we are on the Summary page...
                ...
                page = self.notebook.get_nth_page(
                    page_index
                )  # Get the Gtk.VBox of the selected tab in the logbook.
        ...
        response = auxiliary_dialog.question(
            parent=self.application.window,
    ...
    def render_log(self, index):
        ...
        vbox = Gtk.VBox()
        vbox.set_name(
            self.logs[index].name
        )  # Set a name for the tab itself so we can match it up with the associated Log object later.
    ...
    def sort_log(self, widget, column_index):
        ...
        if self.sorter[log_index].get_sort_column_id()[0] == column_index:
            ...
            if order == Gtk.SortType.ASCENDING:
                self.sorter[log_index].set_sort_column_id(
                    column_index, Gtk.SortType.DESCENDING
                )
                column.set_sort_order(Gtk.SortType.DESCENDING)
            else:
                self.sorter[log_index].set_sort_column_id(
                    column_index, Gtk.SortType.ASCENDING
                )
                column.set_sort_order(Gtk.SortType.ASCENDING)
        else:
    ...
    def import_log(self, widget=None):
        ...
        try:
            ...
            auxiliary_dialog.error(parent=self.application.window, message="Could not import the log.")
            logging.exception(e)
            return
    ...
    def delete_record_callback(self, widget):
        ...
        (sort_model, path) = self.treeselection[
            log_index
        ].get_selected_rows()  # Get the selected row in the log
        try:
